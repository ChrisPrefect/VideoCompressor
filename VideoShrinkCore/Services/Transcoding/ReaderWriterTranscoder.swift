    ...
    public func transcode(
        ...
        videoState.value.input.requestMediaDataWhenReady(on: videoQueue) {
            ...
            while p.input.isReadyForMoreMediaData {
                let step = autoreleasepool { () -> PipelineLoopStep in
                    ...
                    guard let imageBuffer = CMSampleBufferGetImageBuffer(sample) else { return .keepReading }
                    guard let scaledBuffer = pixelScaler.scale(
                        ...
                        return .failed(TranscodingError.pixelBufferCreationFailed)
                    }
                    if !p.adaptor.append(scaledBuffer, withPresentationTime: pts) {
                        ...
                        return .failed(TranscodingError.writerAppendFailed(p.writer.error))
                    }
                    if secs.isFinite {
                ...
                case .failed(let error):
                    p.input.markAsFinished()
                    videoContinuation.yield(.failure(error))
                    videoContinuation.finish()
                    return
                }
            }
        ...
        if let audioInput, let audioOutput {
            ...
            audioState.value.input.requestMediaDataWhenReady(on: audioQueue) {
                ...
                while p.input.isReadyForMoreMediaData {
                    ...
                    case .failed(let error):
                        p.input.markAsFinished()
                        audioContinuation.yield(.failure(error))
                        audioContinuation.finish()
                        return
                    }
                }
...
nonisolated private enum PipelineLoopStep {
    case keepReading
    case finished
    case failed(Error)
}
